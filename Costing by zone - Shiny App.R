...
ui <- shinyUI(fluidPage(theme = shinytheme("cosmo"),
  ...
  # App title ----
  titlePanel("Movia - Costing by municipality"),
  br(),
...
server <- function(input, output, session) {
  ...
  filtered <- reactive({
    ...
    stop_times() %>%
      select(trip_id, MunicipalityCode)
  })
  ...
  trips_ids <- reactive({
    ...
    filtered() %>% 
      ...
      mutate(next_trip = ifelse(is.na(lead(trip_id)), 
                                filtered()[[1, 1]], 
                                lead(trip_id))) %>% 
      #MunicipalityCode of the previous stop
      mutate(prev_zone = ifelse(is.na(lag(MunicipalityCode)),
                                MunicipalityCode,
                                lag(MunicipalityCode)) ) %>% 
      #MunicipalityCode of the next stop
      mutate(next_zone = ifelse(is.na(lead(MunicipalityCode)),
                                MunicipalityCode,
                                lead(MunicipalityCode)) )
  })
  ...
  breaks_1 <- reactive({
    ...
    stop_times()[(
      trips_ids()$trip_id != trips_ids()$prev_trip | 
        ...
        trips_ids()$MunicipalityCode != trips_ids()$prev_zone |
        trips_ids()$MunicipalityCode != trips_ids()$next_zone 
    ), ]
  ...
  breaks_2 <- reactive({
    req(breaks_1())
    breaks_1() %>% 
      filter(is.na(MunicipalityCode) != 'TRUE')
  })
  ...
  breaks <-  reactive({
    ...
    breaks_2() %>% 
      ...
      mutate(travel_hours = ifelse(trip_id == lead(trip_id) & !is.na(lead(trip_id)),round((lead(hours_minutes) - hours_minutes),digits = 2), 0)) %>% 
      select(trip_id, route_id, MunicipalityDisplayName, travel_hours,-timepoint, - direction_id, -hours_minutes) %>% 
      left_join(trips()) %>% 
      select(trip_id, route_id, service_id, MunicipalityDisplayName, travel_hours) %>% 
      left_join(routes()) %>% 
  ...
  yearly_table <- reactive({
    ...
    breaks() %>% 
      ...
      group_by(route, municipality, day_type) %>% 
  ...
  output$downloadYearlyData <- downloadHandler(
    ...
    content = function(file) {
      write_csv(group_selected_yearly(), file)
    }
  )
